...
// written out VERBATIM to tests/golden/fixtures/reference/component.js, wrapped
// in `defineComponent(DCLogic)` so the base class becomes a parameter instead
// of a global.  The tool then imports that written file, so the committed
// artefact is provably the code that produced the fixtures.
//
...
async function loadReference(outDir) {
  ...
  writeText(path.join(refDir, 'component.js'), `${COMPONENT_HEADER}${classBody};\n}\n`);
...
function buildGaTrace(makeRef, harness) {
  const ref = makeRef({ algorithmEffort: 'Fast' });
  const mandate = mandateVariants(ref)[0].mandate;
  ...
  for (let i = 0; i < popSize; i++) {
    initialPopulation.push(pool.map(() => (initRnd() < SPEC_INCLUSION_PROBABILITY ? 1 : 0)));
  }
  ...
  if (!Object.is(initialBest, perGeneration[0].bestFitness) || !Object.is(initialMean, perGeneration[0].meanFitness)) {
    fail(
      'the reconstructed initial population does not reproduce generation 1 of the driven run ' +
      `(best ${initialBest} vs ${perGeneration[0].bestFitness}, mean ${initialMean} vs ${perGeneration[0].meanFitness}). ` +
      'The PRNG consumption order has changed.'
    );
  }
...
async function build(outDir) {
  ...
  const refFiles = ['reference/component.js', 'reference/harness.mjs'];
  ...
  return { artefacts, report, objective, gaTrace, projects, files };
}
...
function summarise(built) {
  const { report, objective, gaTrace, artefacts } = built;
  ...
  lines.push(`  final fitness ${gaTrace.result.fitness}, ${gaTrace.result.selectedCount} projects`);
  lines.push('');
...
async function main(argv) {
  ...
  const built = await build(FIXTURES_DIR);
  writeArtefacts(FIXTURES_DIR, built.artefacts);
  process.stdout.write(`${summarise(built)}\nWritten to ${path.relative(ROOT, FIXTURES_DIR)}/\n`);
  return 0;
}
